...
const setupGame = () => {
  ...
  gameBoard.style.gridTemplateColumns = selectedLevel === "easy" ? "repeat(2, 1fr)" : selectedLevel === "medium" ? "repeat(3, 1fr)" : "repeat(4, 1fr)";
  gameBoard.style.gridTemplateRows = selectedLevel === "easy" ? "repeat(3, 1fr)" : selectedLevel === "medium" ? "repeat(4, 1fr)" : "repeat(4, 1fr)";
  ...
  cards.forEach((image, index) => {
    ...
    card.style.backgroundPosition = "center";
    card.style.backgroundRepeat = "no-repeat";
    card.style.width = "100px"; // Adjusted image size
    card.style.height = "100px"; // Adjusted image size
    card.addEventListener("click", flipCard);
    gameBoard.appendChild(card);
  });
